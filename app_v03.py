...
st.set_page_config(page_title="FKA v03 — EFESO Style (Kostenspalten präzise)", layout="wide")
...
with left:
    logo = st.file_uploader("Logo (optional)", type=["png","jpg","jpeg"], label_visibility="collapsed", key="logo")
    if logo: st.image(logo, use_column_width=True)
    else: st.image("assets/efeso_logo_placeholder.png", use_column_width=True)
with right:
    st.markdown('<div class="efeso-header"><div><p class="efeso-title">FKA — Funktionskosten & Evaluierung</p><p class="efeso-sub">v03 · EFESO-Farben · Präzise Kostenspalten</p></div></div>', unsafe_allow_html=True)
...
def detect_cost_columns(df):
    # Regel 1: Spalten, in deren Headerzeilen (erste 8 Zeilen) ein "€" steht
    header_zone = df.head(8).astype(str)
    euro_cols = [c for c in df.columns if header_zone[c].str.contains("€").any()]
    # Regel 2: Spalten mit Währungsformat/Strings (z. B. "33,85", "11,20") UND überwiegend numerisch nach Parsing
    parsed_numeric_cols = []
    for c in df.columns:
        if c in euro_cols: 
            parsed_numeric_cols.append(c); 
            continue
        # viele Zellen sind numerisch, wenn wir , in . umwandeln
        s = pd.to_numeric(df[c].astype(str).str.replace(".","", regex=False).str.replace(",",".", regex=False), errors="coerce")
        if s.notna().sum() >= max(3, int(0.5*len(s))): # mind. 3 Werte oder 50% numeric
            # verifiziere, dass im Headerbereich zumindest eine Zelle leer oder "€" ist (kein Überschriftstext)
...
def read_product(file):
    ...
    # Detect hierarchy columns (pick first 3 non-numeric, text-heavy)
    text_cols = [c for c in func_df.columns if func_df[c].astype(str).str.len().mean() > 2 and not pd.api.types.is_numeric_dtype(func_df[c])]
    hier_cols = []
    ...
    # fill up to 3 with left-most text columns
    for c in func_df.columns:
        if c not in hier_cols and c in text_cols and len(hier_cols)<3: hier_cols.append(c)

    # COST COLUMNS detection + manual override controls
    candidates = detect_cost_columns(func_df)
    st.session_state.setdefault("cost_map", {})
    with st.expander(f"⚙️ Kostenspalten für {name} (Auto: {len(candidates)} Spalten erkannt)"):
        st.caption("Regel: Spalten mit '€' im Header (erste 8 Zeilen) + überwiegend numerische Werte. Du kannst hier manuell anpassen.")
    ...
    # Build rollups using selected columns
...
products = []
for f in files:
    products.append(read_product(f))
