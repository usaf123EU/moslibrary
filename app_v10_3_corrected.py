# EFESO Functional Cost Analysis TOOLSET
# Version: v10.3-corrected
# Changes vs 10.3: fixed header string literal (robust triple-quoted), keep logic & visuals the same.
...
def xl_col_to_idx(label: str) -> int:
    """Convert Excel column label to 0-based integer index (e.g., 'I' -> 8)."""
    s = label.strip().upper()
    n = 0
    for ch in s:
        n = n * 26 + (ord(ch) - 64)
...
def _clean_num(x):
    """Parse numbers that may include €, %, thousand separators, or commas as decimals."""
    if pd.isna(x):
        return np.nan
...
def parse_product(file_name: str, content: bytes) -> Dict[str, Any]:
    ...
    while col < maxc:
        ...
        if _is_empty(h1_name):
            # stop if also nothing in row2 for a while
        ...
        nxt = col + 1
        while nxt < maxc and _is_empty(df.iat[h1_row, nxt]):
            nxt += 1
        ...
        for c in block_cols:
            ...
            if _is_empty(h2_name):
                continue
            h2s.append({
                "name": str(h2_name).strip(),
    ...
    if tname in xl.sheet_names:
        ...
        for r in range(tdf.shape[0]):
            name = tdf.iat[r, 1] if 1 < tdf.shape[1] else None  # Column B
            score = tdf.iat[r, 17] if 17 < tdf.shape[1] else None  # Column R
            if not _is_empty(name):
                tech[str(name).strip()] = _clean_num(score)

    # Build DataFrames
    h1_costs = pd.DataFrame(
        [{"H1": h["name"], "Kosten_H1": h["cost"], "Gewichtung_H1_%": h["weight"]} for h in h1_blocks]
    ).dropna(how="all")
    ...
    # Weighted H1 tech = sum(H2_score * H2_share%)
    ...
    if not h2_costs.empty:
        h1_tech_weighted = (
            h2_costs.merge(h2_tech[["H1", "H2", "TechScore"]], on=["H1", "H2"], how="left")
                     .groupby("H1")
                     .apply(_w_h1).reset_index(name="TechScore_H1_weighted")
        )
    else:
...
st.markdown("""
# EFESO - Functional Cost Analysis TOOLSET
**Version v10.3 (corrected)** · Parser konsolidiert · Swimlanes ohne Farben · Dünne Balken · Mehrprodukt-Linienchart
""")


uploaded = st.file_uploader(
    "Excel-Dateien (.xlsx/.xlsm) - je Produkt eine Datei", type=["xlsx", "xlsm"], accept_multiple_files=True
)
...
with tabs[0]:
    ...
    # Order columns by H1 total cost (desc) for readability
    h1_blocks_sorted = sorted(
        pdata["h1_blocks"], key=lambda x: (x["cost"] if x["cost"] is not None else -1), reverse=True
    )
    ...
    for row in rows:
        ...
        for c, h1 in zip(cols, row):
            with c:
                ...
                h2s_sorted = sorted(
                    h1["h2"], key=lambda x: (x["weight"] if x["weight"] is not None else -1), reverse=True
                )
                for h2 in h2s_sorted:
                    pct = f"{h2['weight']:.0f}%" if pd.notna(h2["weight"]) else ""
                    st.markdown(
                        f"<div class='tile'><span class='name'>{h2['name']}</span><span class='pct'>{pct}</span></div>",
                        unsafe_allow_html=True,
                    )
                st.markdown("</div>", unsafe_allow_html=True)

    st.markdown("**Beschreibung / Kommentare - Funktionsmatrix**")
    st.text_area("", key="comment_matrix", height=120, placeholder="Ihre Notizen…")
...
with tabs[1]:
    ...
    # Thin bars via bargap/groupgap
    if not pdata["h1_costs"].empty:
        fig1 = px.bar(
            pdata["h1_costs"].sort_values("Kosten_H1", ascending=False),
            x="H1", y="Kosten_H1", title="Kosten je Hauptfunktion (Zeile 7)"
        )
...
with tabs[2]:
    ...
    st.caption("Alle hochgeladenen Produkte in einem Linienchart. Y = Tech-Score (Spalte R), X = H2.")
    ...
    if blocks:
        ...
        order = (
            tech_all.groupby("H2")["TechScore"].mean().sort_values(ascending=False).index.tolist()
        )
    ...
    st.divider()
    st.subheader("Technische Bewertung - aggregiert nach Hauptfunktion (gewichteter Score)")
    prod = st.selectbox("Produkt wählen", product_names, key="sel_prod_tech_agg")
    pdata = st.session_state.products[prod]
    if not pdata["h1_tech_weighted"].empty:
        fig3 = px.bar(
            pdata["h1_tech_weighted"].sort_values("TechScore_H1_weighted", ascending=False),
            x="H1", y="TechScore_H1_weighted"
        )
        fig3.update_layout(bargap=0.6, bargroupgap=0.7, showlegend=False)
...
st.markdown(
    "<br><hr><small>© EFESO · Version v10.3-corrected · Vorlage für Funktions- & Kostenanalyse</small>",
    unsafe_allow_html=True,
)
