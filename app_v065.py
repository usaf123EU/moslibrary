...
st.set_page_config(page_title="FKA v06.5 — EFESO Style", layout="wide")
...
st.markdown('<div class="sticky-header"><h1>FKA — Funktionskosten & Evaluierung</h1><p>v06.5 · H2-only · H1 = Summe(H2) · Zeile1/2-Namen</p></div>', unsafe_allow_html=True)
...
def parse_h1_h2_from_header(func_df):
    ...
    # auto-detect first non-empty H1 column
    start_col = None
    for c in range(ncols):
        if _cell_str(func_df, HAUPTFUNKTION_NAME_ROW, c):
            start_col = c
            break
...
with tab1:
    ...
    if not P["H1"].empty:
        fig = px.bar(P["H1"], x="Hauptfunktion", y="Kosten Hauptfunktion")
        fig.update_layout(xaxis=dict(tickangle=0, tickfont=dict(size=11)),
                          yaxis=dict(title="Kosten [€]", showgrid=True, gridcolor="LightGrey"),
                          plot_bgcolor="white", margin=dict(b=100))
    ...
    if not P["H2"].empty:
        ...
        fig2 = px.bar(h2, x="Nebenfunktion", y="Anteil_%")
        fig2.update_layout(xaxis=dict(tickangle=0, tickfont=dict(size=11)),
                           yaxis=dict(title="Anteil [%]", showgrid=True, gridcolor="LightGrey"),
...
with tab2:
    ...
    with c1:
        fig = px.bar(P["H1"], x="Hauptfunktion", y="Kosten Hauptfunktion", title="Kosten je Hauptfunktion")
        fig.update_layout(xaxis=dict(tickangle=0, tickfont=dict(size=11)),
                          yaxis=dict(title="Kosten [€]", showgrid=True, gridcolor="LightGrey"),
                          plot_bgcolor="white", margin=dict(b=120), title_x=0.02)
        st.plotly_chart(fig, use_container_width=True)
    with c2:
        tinfo = P["tech"]
        ...
        if not ttable.empty:
            figt = px.bar(ttable, x="Funktion", y="Gewichteter Score")
            figt.update_layout(xaxis=dict(tickangle=0, tickfont=dict(size=10)),
                               yaxis=dict(title="Gewichteter Score", showgrid=True, gridcolor="LightGrey"),
                               plot_bgcolor="white", margin=dict(b=160))
...
with tab3:
    ...
    if len(products)<2:
        ...
        if a==b: st.warning("Bitte zwei unterschiedliche Produkte wählen.")
            ...
            figc = px.bar(dfm, x="Hauptfunktion", y="Kosten", color="Produkt", barmode="group",
                          color_discrete_map={a:"#0055A4", b:"#66B2FF"})
            figc.update_layout(xaxis=dict(tickangle=0, tickfont=dict(size=11)),
                               yaxis=dict(title="Kosten [€]", showgrid=True, gridcolor="LightGrey"),
...
with tab4:
    ...
    if len(products)<2:
        ...
        if a==b: st.warning("Bitte zwei unterschiedliche Produkte im Tab 3 wählen.")
            ...
            colors = np.where(top["Delta (B - A)"] >= 0, "#0055A4", "#f47c24")
            fig = go.Figure(go.Bar(x=top["Label"], y=top["Delta (B - A)"], marker_color=colors,
                                   hovertext=top["key"], hovertemplate="Δ (B−A): %{y:.2f} €<extra></extra>"))
            fig.update_layout(
                xaxis=dict(tickangle=0, categoryorder='array', categoryarray=top["Label"].tolist(), tickfont=dict(size=11)),
                yaxis=dict(title="Δ Kosten [€] (B−A)", zeroline=True, showgrid=True, gridcolor="LightGrey"),
                plot_bgcolor="white", margin=dict(l=40,r=20,t=10,b=110), showlegend=False
            )
            st.plotly_chart(fig, use_container_width=True)
