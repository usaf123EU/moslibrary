...
# --- Logo embedding (correct quoting) ---
def _efeso_logo_b64():
...
<div class="headerbar">
  {logo_html}
  <div>
    <h1>FKA — Funktionskosten & Evaluierung <small>v07.3-stable</small></h1>
...
# --- SLAVE Header rows ---
HAUPTFUNKTION_NAME_ROW = 0   # Zeile 1
NEBENFUNKTION_NAME_ROW = 1   # Zeile 2
NEBENFUNKTION_WERT_ROW = 4   # Zeile 5
...
tab1, tab2, tab3, tab4, tab5 = st.tabs([
    "1) Überblick", "2) Kosten & Tech (H1)", "3) Vergleich A vs B",
    "4) Top-Abweichungen (H2)", "5) Kosten vs Gewichtung"
])
...
with tab1:
    ...
    if not P["H2"].empty:
        ...
        st.altair_chart(alt_bar(h2,"Nebenfunktion:N","Anteil_%:Q","Anteile Nebenfunktionen (%)"),
...
with tab2:
    ...
    with c1:
        P_h1=ensure_h1_costs(P)
        st.altair_chart(alt_bar(P_h1,"Hauptfunktion:N","Kosten Hauptfunktion:Q","Kosten je Hauptfunktion"),
                        use_container_width=True)
    with c2:
        ttable=P["tech"]["table"]
        if not ttable.empty and not P_h1.empty:
            ...
            if not ttf.empty:
                ...
                st.dataframe(ttf[["Funktion","Gewichtung_%","Score","Gewichteter Score"]],
